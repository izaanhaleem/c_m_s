...
class CMSResolvedComplaint extends StatefulWidget {
  const CMSResolvedComplaint({Key? key}) : super(key: key);

  @override
  _CMSResolvedComplaintState createState() => _CMSResolvedComplaintState();
}

class _CMSResolvedComplaintState extends State<CMSResolvedComplaint> {
  ...
  void initState() {
    Future.delayed(Duration(seconds: 0),
        () => setState((){
      parentprofilecontroller.CmsComplainResolvetList();
      parentprofilecontroller.CmsComplainResolvetList.length;
        })
    );
    TabControll();
    super.initState();
  ...
  Widget Title(){
    return const Text("Resolved Complaints",style: TextStyle(fontSize: 20,fontWeight: FontWeight.bold),);
  }
  Widget TabControll(){
   ...
   Widget AllResolvedComplaintList(){
    return Column(
      children: [
        Obx((){
          if (parentprofilecontroller.CmsComplainResolvetList.isEmpty) {
            return const Center(child: Text("No Assign Resolved Complaint List"),);
          } else {
            return ListView.builder(
                ...
                itemBuilder: (BuildContext context, int index) {
                  return ResolvedComplaintlist(index, parentprofilecontroller.CmsComplainResolvetList);
                }
            );
          }
        })
  ...
  Widget ResolvedComplaintlist(int index, RxList<CMSComplaintList> cmsComplaintList) {
    return GestureDetector(
      onTap: () async{
        ComplaintId = parentprofilecontroller.CmsComplainResolvetList[index].complaintId;
        SharedPreferences pref = await SharedPreferences.getInstance();
        pref.setInt('complaintId',ComplaintId!);
        Navigator.of(context).pushAndRemoveUntil(MaterialPageRoute(
            builder: (context) =>ComplaintStatus(
                ComplaintId: ComplaintId!)),(Route<dynamic> route)=>false);
      },
      child: Card(
        color: Colors.green,
